                ...
                <Box sx={{ display: "flex", gap: 3, mb: 6, justifyContent: "center", flexWrap: "wrap" }}>
                    {["Placement", "Result", "Event"].map((type) => (
                        <Box key={type} sx={{
                            ...
                            <Typography sx={{ color: "#fff", fontWeight: 800, fontSize: "2.5rem" }}>
                                {notifications.filter((n) => n.Type === type).length}
                            </Typography>
                            <Typography sx={{ color: "rgba(255,255,255,0.5)", fontSize: "1.1rem", textTransform: "uppercase", letterSpacing: "1px", fontWeight: 600 }}>{type}</Typography>
                        </Box>
                    ))}
                    <Box sx={{
